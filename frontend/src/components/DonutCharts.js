...
const PentagonIcon = ({setCat, hoveredPair, setHoveredPair }) => {
    const icon_list = [
    ...
    const pentagon = Array.from({ length: 5 }).map((_, index) => (
      <div key={index} className={hoveredPair===index ? `iconStructure${index}-hovered` : 'iconStructure'} 
           style={{...pentagonStyle, transform: `translate(${calculatePosition(index, 0).x * (100/size) }%, ${calculatePosition(index, 0).y  * (100/size)}%)`}}
...
const DonutCharts = ({data, backgroundColor, setCat, hoveredPair, setHoveredPair}) => {
    ...
    return (
        ...
        <div className="donut_pentagon_chart">
            ...
            <div className="donut_pentagon_chart_inner_icon">
                <PentagonIcon setCat={setCat} hoveredPair={hoveredPair} setHoveredPair={setHoveredPair} />
            </div>
        </div>
